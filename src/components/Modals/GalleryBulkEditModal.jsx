...
export default function GalleryBulkEditModal({
  selectedCount,
  selectedGalleries,
  allGalleryTags,
  onClose,
  ...
  // Extract unique existing tags from selected galleries
  const existingTags = React.useMemo(() => {
    if (!selectedGalleries || selectedGalleries.length === 0) return [];
  ...
  const getTagSuggestions = () => {
    ...
    const unusedTags = allGalleryTags.filter(tag => !bulkTagsToAdd.includes(tag.toLowerCase()));
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold">Bulk Edit {selectedCount} Galler{selectedCount > 1 ? 'ies' : 'y'}</h2>
          <button
            onClick={onClose}
        ...
        <div className="mb-6">
          <label className="flex items-center gap-2 text-sm font-semibold mb-2">
            <Tag size={16} />
            Add Tags to Selected Galleries
          </label>
        ...
        >
          <Trash2 size={20} />
          Delete {selectedCount} Galler{selectedCount > 1 ? 'ies' : 'y'}
        </button>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
            <div className="bg-gray-800 rounded-xl p-6 max-w-md w-full mx-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-xl font-bold">Delete {selectedCount} Galler{selectedCount > 1 ? 'ies' : 'y'}?</h3>
                  <p className="text-sm text-gray-400">This action cannot be undone</p>
                </div>
              </div>
              <p className="text-gray-300 mb-6">
                Are you sure you want to permanently delete {selectedCount === 1 ? 'this gallery' : `these ${selectedCount} galleries`} and all their images?
              </p>
              <div className="flex gap-3">
                <button
